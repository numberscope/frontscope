...
As on the [previous page](visualizer-basics.md), let's begin with a diagram of
the life cycle of a visualizer. Only this time, we've attempted to capture all
of the methods that your Visualizer class might want to implement, extend, or
override to accomplish a variety of effects, as well as all of the significant
actions that might occur as someone explores sequences with your

[NEED TO ADD EVENTS/HANDLERS TO THIS DIAGRAM] [AND CONVERT TO D2]
...
this time in the order of the diagram. Information already on the
[introductory page](visualizer-basics.md) is not repeated here, but there is
additional detail on most of the items.
...
copy the new values back into the copies used for controlling the user
...
The third provides an opportunity to check global consistency among many
...
in [`setup()`](#set-up-for-drawing), as described below. At that point, errors
you add to the parameters or the visualizer's `validationStatus` won't block
the new parameters from taking effect as they would if handled in
checkParameters(), but at least the person running the visualization will be
notified in the UI. Another fine point here is that since `validationStatus`
and `statusOf` properties are not by default reset when `setup()` is again
called, you may need to take steps to ensure that additional copies of your
error or warning do not accumulate as `setup()` is repeatedly called (say for
the sake of resizing the screen).
...
set-up functions discussed [below](#set-up-for-drawing).
...
It is important to know that Vue will instrument (insert code into) your
...
your class. That call tells Vue not to instrument that value at all. It means
that changes to that object cannot directly affect the browser view; but
presumably you are already calling `refreshParams()` to explicitly update the
view when needed, so there is unlikely to be a problem. Note, however, that
you should not call `markRaw` on the values of _parameters_ even if they are
objects as in the case of `ParamType.NUMBER_ARRAY`; doing so could prevent
proper display updating.
...
do not need to modify that behavior. However, if for example you are
progressively drawing a diagram and would like to simply continue in the
middle of drawing with a newly-specified color when its parameter changes
(rather than re-drawing the entire thing from the beginning with that new
color), you could implement an altered `parametersChanged()` that would detect
the situation and avoid the reset.
