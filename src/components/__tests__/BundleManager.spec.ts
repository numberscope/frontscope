import {describe} from 'vitest'
// import {mount} from '@vue/test-utils'
// import BundleManager from '../BundleManager.vue'

describe.todo('BundleManager', () => {
    /*
    If we try to import BundleManager, we get an error when we do
    `pnpm test:unit`:

    TypeError: document.hasFocus is not a function
    ❯ Object.254../constants node_modules/p5/lib/p5.min.js:3:401058

    The test suite isn't providing the document object. I'll leave this file 
    as a placeholder. Maybe we'll figure out a way to test this in the future.

    The dead code below is part of what we should test if/when we can import
    BundleManager.
    */
    // it('emits createBundle when Create Bundle button is clicked', () => {
    //     const wrapper = mount(BundleManager)
    //     wrapper.find('button').trigger('click')
    //     expect(wrapper.emitted()).toHaveProperty('createButton')
    // })
})
