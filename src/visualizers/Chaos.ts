...
It is also possible to configure a more complex formula for the step the
walker takes, not just along a straight line toward the corner.  This 
makes the visualizer capable of drawing an arbitrary iterated function
system on the plane.

When this process is performed with a random sequence, this has been called 
the 'chaos game.'  The chaos game on a square produces a uniformly coloured
...
- **Static mode**: When checked, turns off zoom/pan and removes the
  iteration limit, allowing for more performant images.
   **/
  staticMode: {
    default: false,
...
Unless *Static mode* is turned on, you may click and drag to pan the view, and use the scroll wheel to zoom
in and out.
 **/
